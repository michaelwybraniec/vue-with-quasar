...
const mutations = {
    ...
    [ADD_REMOVE_FAV_HERO](state, favoriteHero) {
        ...
        if (!matchingHero) {
            ...
            state.favorite_heroes = remove(state.favorite_heroes, favoriteHero);
    }
};
...
const actions = {
    async getHeroAction(context, input) {
        ...
        try {
            let heroById = await axios.get(reqByIdUrl);
            if (!heroById.data.error) {
                if (heroById.data.response === 'error') {
                    ...
                    heroById.data.favorite = !!dataService.favoriteHero(
                        heroById.data.id,
                        this.state
                    );
                    heroById.data.powerstars = dataService.statsToStars(
                        heroById.data.powerstats
                    );
                    context.commit(API_ERROR, undefined);
                    context.commit(ADD_HERO, heroById.data);
                }
            } else if (heroById.data.error) {
                let heroByName = await axios.get(reqByNameUrl);
